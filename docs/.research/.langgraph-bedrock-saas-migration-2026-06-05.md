...
| UI framework | Custom React + FastAPI with a **Chainlit-inspired** design system (shadcn/ui + Tailwind + Inter + CSS custom-property tokens) shared between user-facing Studio and admin Atlas Console |
...
| Telemetry | Clickstream events (35-event taxonomy) + admin SPA with Initiative / Application / Repository / Domain rollups |
...
5. [Atlas Console (admin dashboard) design](#5-atlas-console-admin-dashboard-design)
...
| **Atlas Console SPA** | Admin dashboard (`/admin/*` routes in the same bundle) | Same CloudFront | Same nginx |
...
Tokens land as CSS custom properties in `src/lib/theme.ts` so both Studio and Atlas Console inherit them and tenants can re-skin without code.
...
/admin/*                  Atlas Console (see §5)
/auth/callback            Cognito redirect
```

---

## 5. Atlas Console (admin dashboard) design
...
| `taxonomy <project>` | Interactive wizard: assigns the migrated project to Initiative / Application / Domain. Atlas Console has the same wizard for ongoing assignments. |
...
8. **Taxonomy assignment UX.** Migration wizard at first push; Atlas Console "Untagged" view nags admins; new projects in Studio require selection.
...
7. ✅ The 35-event taxonomy covers every numbered step in every upstream skill (§8.2 + `event_schema/registry.md`).
...
| `packages/event-schema/` | Pydantic envelope + 35 typed payloads + registry doc |
...
| `apps/studio/` | React + Vite + Tailwind + shadcn/ui + Chainlit-inspired tokens; Studio + Atlas Console in one bundle |
