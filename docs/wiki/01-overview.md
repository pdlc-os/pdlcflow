...
| **pdlc-graph** | `packages/pdlc-graph/` | The LangGraph engine: meta-graph router + `meta/brainstorm/build/ship/night_shift/utility` subgraphs, party orchestrator, 10 persona soul-specs, the 9 gates, the deterministic Sentinel evaluator, and dep-free injectable ports (LLM, tracing, prompt-resolver, tool/MCP). |
