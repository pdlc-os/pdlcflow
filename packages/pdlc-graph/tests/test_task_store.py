...
def test_create_mints_bd_ids_and_records_org():
    ...
    rows = {t["external_id"]: t for t in s.list("o", "p1")}
...
def test_create_preserves_supplied_external_id():
    ...
    assert ext == "bd-42"
    assert s.list("o", "p1")[0]["external_id"] == "bd-42"
...
def test_add_dependency_records_blockers():
    ...
    s.create("o", "p1", "a", "", [])
    s.create("o", "p1", "b", "", [])
    s.add_dependency("o", "p1", "bd-1", "bd-2")
    rows = {t["external_id"]: t for t in s.list("o", "p1")}
...
def test_claim_is_atomic_and_branch_unique():
    ...
    s.create("o", "p1", "a", "", [])
    s.create("o", "p1", "b", "", [])
    assert s.claim("o", "p1", "bd-1", "feat/x", "dev@acme") is True
    # Same branch can't be claimed by another task in the project.
    assert s.claim("o", "p1", "bd-2", "feat/x", "dev@acme") is False
    # Already-claimed task can't be re-claimed.
    assert s.claim("o", "p1", "bd-1", "feat/y", "dev@acme") is False
    rows = {t["external_id"]: t for t in s.list("o", "p1")}
