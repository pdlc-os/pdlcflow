...
- [x] `apps/studio` — React + Vite + Tailwind + shadcn/ui + Chainlit-inspired tokens (CSS custom properties, light/dark with system-preference detection), Studio + Nexus Console in one bundle, 12 stub components (`<ChatPanel>`, `<StepCard>`, `<SideDrawer>`, `<SettingsDrawer>`, `<StatusLine>`, `<ThemeToggle>`, `<PartyMeetingVisualizer>`, `<ApprovalGateModal>`, `<MemoryFileViewer>`, `<NightShiftMissionControl>`, `<RoadmapBoard>`, `<SketchSocraticToggle>`), 8 Nexus Console routes (Live, Initiatives, Domains, Squads, Agents, Features time-travel, Exports, Models), Zustand stores, WS client with auto-reconnect, typed API client
...
- [x] **Nexus Console SPA**: all 7 pages render the rollups (tables + Recharts bar charts; CSV export link; live feed auto-refresh), org-scoped via the session store.
...
- [x] Verified live: `scan → push (9 memory/4 tasks/2 decisions/1 deploy) → backfill (8 events) → re-push (0 new, idempotent) → /v1/admin/live` shows the imported history. **Backfill makes Nexus Console non-empty on day one** (plan §12), via the live feed + roadmap/domain drill-downs.
